...
class MyRadioList{
  ...
  Map<String,dynamic> toMap(){
    return {
      'radios':radios.map((e) => e.toMap()).toList(),
    };
  }
  factory MyRadioList.fromMap(Map<String,dynamic> map){
    if (map==null) ;
    return MyRadioList(
      radios:List<MyRadio>.from(map['radios']?.map((e)=>MyRadio.fromMap(e))),
    );
  }
...
class MyRadio{
  ...
  final String image;
  final String lang;
  MyRadio({required this.id,required this.order, required this.name, required this.tagline, required this.color, required this.desc, required this.url, required this.category, required this.icon, required this.image, required this.lang,
